...
ggplot( diamonds_sample )   # notice this displays nothing
...
ggplot( 
  ...
  mapping = aes( x = clarity, y = mean_price ) 
  ) +
  geom_bar( 
  ...
  geom_errorbar( 
    mapping = aes( ymin = mean_price - stderr_price, ymax = mean_price + stderr_price ),
    size = 1.5
  )
